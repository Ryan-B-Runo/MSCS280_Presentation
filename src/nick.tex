...
\begin{document}
    ...
    For a graph $K_{1, n-1},\ n \geq 2$, it follows that in radial coordinates, all vertices must exists at distinct degrees.\\
    That is, $\exists \theta_i\ \forall 0\leq i, k < n-1 \in \Z|\ \theta_i \neq \theta_k$ and $\forall\theta _i, \exists\text{ radius }r_i$.\\
    ...
    \begin{equation}
        v_i = (r_i \cos\left(\frac{2\pi}{n-1} i\right), r_i \sin\left(\frac{2\pi}{n-1}i\right));\ r_i \neq 0
    \end{equation} with edges:
    \begin{equation}
        E(v_{(0,0)}, v_i) = (t\cdot r_i \cos\left(\frac{2\pi}{n-1} i \right), t\cdot r_i\sin\left(\frac{2\pi}{n-1} i\right) );\ 0\leq t\leq1\in \R
    \end{equation}
    Thus, by construction, $K_{1, n-1}$ n-finite is planar.
    \begin{figure}[htbp]
        \centering %\linewidth or scale=.3
        \includegraphics[height=.25\textheight]{Screenshot 2025-11-13 005615.png}
        \caption{$K_{1, n-1}$ of monotonically increasing radii}
        \label{fig:lec3-shot}
    \end{figure}

    \subsection{Planarity of $K_{1, \infty}$}
    The definition of planar requires edges to intesect only at their endpoints.\\
    ...
    That is, $r_0\leq r_1 \leq r_2 \leq \dots \leq r_n$. \\
    Pick any two distinct vertices in $K_{1, n-1}$. \\
    That is, let $k, c \in \Z |\ 0\leq k<n-1,\ 0<c<n-1-k$. \\
    It follows that the edges created from the images of the vertices $v_k$ and $v_{k+c}$ may only intersect when the radii are the same. That is, $t_{k+c} = \frac{r_{k}}{r_{k+c}} t_{k}|\ 0 < \frac{r_k}{r_{k+c}} t_{k} \leq 1 $.\\
    May we evaluate $E(v_{(0, 0)}, v_i) = E(v_{(0, 0)}, v_{i+c})$:
    \begin{equation}
        t_k\cdot r_k e^{i\frac{2\pi}{n-1}k} = t_{k+c}\cdot r_{k+c}\ e^{i\frac{2\pi}{n-1}(k+c)}
    \end{equation}
    It follows that:
    \begin{align}
        t_k\cdot r_k e^{i\frac{2\pi}{n-1}k} - t_{k+c}\cdot r_{k+c}\ e^{i\frac{2\pi}{n-1}(k+c)} &= 0 \\
        r_{k+c} \cdot (\frac{r_k}{r_{k+c}}t_k e^{i\frac{2\pi}{n-1} k} - t_{k+c} e^{i\frac{2\pi}{n-1}} (k+c)) &= 0 \\
        r_{k+c} \cdot (t_{k+c} e^{i\frac{2\pi}{n-1} k} - t_{k+c} e^{i\frac{2\pi}{n-1} (k+c)}) &= 0 \\
        t_{k+c} \cdot r_{k+c} \cdot (e^{i\frac{2\pi}{n-1} k} - e^{i\frac{2\pi}{n-1} k} e^{i\frac{2\pi}{n-1}c}) &= 0 \\
        t_{k+c} \cdot r_{k+c} \cdot e^{i\frac{2\pi}{n-1} k} \cdot (1-e^{i\frac{2\pi}{n-1} c}) &= 0
    \end{align}
    ...
    That is, no finite n allows for the fufilment of the above equality.

    \subsubsection{Infinite Graphs}

    The order of a Graph G is the cardinality of the vertex set. That is $|V(G)|=n$. A graph is then infinite when $|V(G)|$ or $|E(G)|$ is infinite. A popular method to organize a countably infinite graph G is via an increasing union of finite subgraphs$^{[\text{a, 8.1, 229}]}$. \\
    ...
    \end{align}
    This method of organization, however, is not true for all graph sequences, but can be meaningful to be able to describe the behavior of a graph as it approaches a larger limit graph. One trivial example is to merely assume a graph whose edges change in respect to the modulo of the order. When described as such, every finite induced subgraph of $G_\infty$ is in some $G_n\ ^{[\text{a, 8.1, 233}]}$. Allowing $G_\infty$ to be percieved as a graph of arbitrary scale. Unsuprisingly, it follows that if $G_\infty$ of nested $G_n$ is planar for all n, $G_{\infty}$ is also planar as there is no induced subgraph that can contradict planarity. As it relates to planarity, the far more interesting object is the topological closure of an infinite graph. Denoted via a bar over the top, the closure of an infinite graph is the union of the infinite graph and it's limit points$^{[b, \text{Theorem 17.6}]}$. Limit points are the set of points approached as sets approach infinity, with some points being a literal representation of $\infty$, ex: $\overline{\N}=\N\cup\{\infty\}$. Graphs, however, are very odd objects; being merely a collection of vertices, without inherent spacial coordinates, and edges, being two tuples of vertices. To which, the closure of an infinite graph is merely that of the graph with an additional vertex: $v_\infty$, or vectices, all with appropriate edges. Notice that as $\overline{\bigcup _{m=1} ^{n} G_m} = \bigcup_{m=1} ^{n} \overline{G_m}$ for n-finite and $G_m$ being subsets of $G_n$, limit vertices and edges must be added to each appropriate finite and infinite subgraph, and if there are infinitely many subgraphs, as may occur with some trees or many constructions, an additional subgraph $G_{n\to\infty}$ may itself be needed. \\
    Planarity is a property of the image of a graph. For the closure of an infinite graph to then be planar, it is self evident the following equality must hold:
    \begin{equation}
        \Psi(\overline{G_\infty}) = \overline{\Psi(G_\infty)}
    \end{equation}
    That is, $\Phi: G_m \to \R^2$ must be a topological embedding$^{[\text{c, Theorem 3.51}]}$. A topological embedding f for topological spaces $(X, T), (Y, T')$ is a homeomorphism from X to Y$^{[d,\text{MunkTop.18.4}]}$. A homeomorphism is a bijection (one-to-one and onto) between topologies: $f: A\to B$, where both $f$ and $f^{-1}$ are continuous $^{[b,\ 105]}$.
    $(X, T)$ is a topological space iff T is a topology on X$^{[\text{d, MunkTop.12.2}]}$. \\
    T is a topology on X iff:$^{[\text{d, MunkTop.12.1}]}$
    \begin{indented}
        1. $T\subseteq \wp(X)$ ($\wp(X)$ is the power set from discrete mathematics)\\
        2. $\emptyset \in T$ and $X \in T$ \\
        3. $\forall S\subseteq T,\ \cup S \in T$ \\
        4. $\forall U, V \in T,\ U \cap V \in T$
    \end{indented}
    Notice: T is a set of sets and $\wp(S)$ of any set is a topology.\\\\
    Or in other words, as $(X, \wp(X))$ is a topological space, a topological embedding is just a function between two domains that is one-to-one, onto, and not discontinous.

    \subsubsection{Planarity of $\overline{K_{1, \infty}}$}

    Let's revist our chosen construction for $K_{1, n-1}$:
    \begin{align}
        \Psi(V(K_{1,n-1})) &= \left\{(r_0, 0), (r_1, \frac{2\pi}{n-1}),  \dots, (r_{n-2}, \frac{2\pi}{n-1}(n-2))\right\}\\
        \Psi(E(K_{1,n-1})) &= \left\{(r_0t_0, 0), (r_1 t_1, \frac{2\pi}{n-1}),  \dots, (r_{n-2}t_{n-2}, \frac{2\pi}{n-1}(n-2))\right\}
    \end{align}
    \\\\
    $[$a$]$ - \url{https://www.math.uni-hamburg.de/home/diestel/books/graph.theory/preview/Ch8.pdf}\\
    $[$b$]$ - \url{https://www.teachmint.com/tfile/studymaterial/b-sc/topology/topologybookbyjrmunkres/8ff79f20-460e-4c35-ba0e-7031a6dfac65} \\
    $[$c$]$ - \url{https://tisp.indigits.com/metric_spaces/continuity} \\
    $[$d$]$ - \url{https://www.andrew.cmu.edu/user/avigad/Papers/mkm/munkres.pdf} \\
    Sorry, I'll have to get these moved to the references. I used letters so they're easier to replace.
    \newpage
